"""Nox sessions for Alibaba Cloud Magic Modules CI."""
...
COLLECTION_NAMESPACE = "stevefulme1"
COLLECTION_NAME = "alicloud"
...
def sanity(session: nox.Session) -> None:
    ...
    (mu_dir / "alicloud_common.py").write_text(
        "class AliCloudModuleBase:\n    pass\n",
        encoding="utf-8",
    )
    ...
    session.run(
        "ansible-test", "sanity",
        "--python", "3.12",
        "--skip-test", "import",           # alicloud deps not installed
        "--skip-test", "pylint",           # stubs won't satisfy pylint
        "--skip-test", "validate-modules", # doc_fragments require real alicloud collection
        "--skip-test", "ansible-doc",      # doc_fragments require real alicloud collection
